...
architecture implementation of Gateway_Logik_v1_0_M00_AXI is
    ...
    signal last_araddr : std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0);
	   	        ...
	   	        -- axi_araddr <= std_logic_vector (unsigned(axi_araddr) + 4);
	   	          ...
	   	          if can_enabled_status = '0' then 
	   	           if unsigned(read_index) = 1 then                                                               --constant STATUS_REG: std_logic_vect
                        ...
                        axi_araddr <= ISR;                                                              --constant TXFIFO_ID : std_logic_vect
                    ...
                    --elsif unsigned(read_index) = 3 then                                                                            --constant TXFIFO_DLC : std_logic_vec
                       ...
                       axi_araddr <= RXFIFO_ID;                                                                    --constant TXFIFO_DW1 : std_logic_vec
                    elsif unsigned(read_index) = 4 then                             --SRR_CAN_ENABLE      --constant TXFIFO_DW2 : std_logic_vec
                        axi_araddr <= RXFIFO_DLC; 
                    elsif unsigned(read_index) = 5 then
                        axi_araddr <= RXFIFO_DW1;                     
                    elsif unsigned(read_index) = 6 then
	      ...
	      if (rising_edge (M_AXI_ACLK)) then                                               
	        if (M_AXI_ARESETN = '0' or init_txn_pulse = '1' ) then                                                 
	          ...
	          case last_araddr is
                    ...
                    when RXFIFO_ID  => CAN0_FRAME_IsReceived_ID  <= M_AXI_RDATA;
                    when RXFIFO_DW1 => CAN0_FRAME_IsReceived_DW1 <= M_AXI_RDATA;
                    when RXFIFO_DW2 => CAN0_FRAME_IsReceived_DW2 <= M_AXI_RDATA;
                    when ISR =>
                      ...
                      -- your clean_icr / TX_INTERRUPT_TO_CAN1 logic here
                    when others => null;
                  end case;
                end if;
              --end if;
