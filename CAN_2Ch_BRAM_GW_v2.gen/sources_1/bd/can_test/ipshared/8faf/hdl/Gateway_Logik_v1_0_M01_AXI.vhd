...
entity Gateway_Logik_v1_0_M01_AXI is
	generic (
    ...
    -- and read transactions the master will perform as a part of this example memory test.
		C_M_TRANSACTIONS_NUM	: integer	:= 5
...
end Gateway_Logik_v1_0_M01_AXI;

architecture implementation of Gateway_Logik_v1_0_M01_AXI is
	...
	constant ISR_STAT : std_logic_vector (31 downto 0) := x"0000001C"; -- interrupt status register 
...
begin
 ...
 --  CAN_FRAME_IsReceived_DW2 <=can_rx_dw2 ;
    ...
    can1_enabled_stat_led <= can_enabled_status;
	    	...
	    	if (rising_edge (M_AXI_ACLK)) then                                              
	    	  if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                
			    ...
			    if can_enabled_status = '0' then 
			     led_vector_can1 <= "01";   	                                                                                                   --constant BTR : std_logic_vector (31	    	    		                                                                                       --constant SR : std_logic_vector (31 
                ...
                elsif (can_enabled_status = '1' and TX_INTERRUPT_FROM_CAN0 = '1') then
                    led_vector_can1 <= "11";
                       if unsigned(write_index) = 1 then                                                               --constant TXFIFO_DW2 : std_logic_vec
                           ...
                           axi_awaddr <= TXFIFO_DW2;                                                                       --   constant ADDR_TXHPB_ID  : std_lo                                                                 --   constant ADDR_TXHPB_DW1 : std_lo
		  ...
		  begin                                                                             
		    if (rising_edge (M_AXI_ACLK)) then                                              
		      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                
                ...
                if (TX_INTERRUPT_FROM_CAN0 = '0') then                                                   --MSR_NORMAL_MODE
...
--                        axi_wdata <= CAN_DATA2;                                --CAN_DATA2    :                                --CAN_DATA2    :           --   constant ADDR_TXHPB_ID  : std_lo                                                                 --   constant ADDR_TXHPB_DW1 : std_lo
--                    else                                                         --CAN_DATA2    :      --   constant ADDR_TXHPB_DW2 : std_lo
--                        axi_wdata <= x"10101010";                                --CAN_DATA2    :      --   constant ADDR_TXHPB_DW2 : std_lo
--                    end if;   
                    
                elsif (TX_INTERRUPT_FROM_CAN0 = '1') then
                    if unsigned(write_index) = 1 then                             --BRP_VAL :  std_     --constant TXFIFO_DW2 : std_logic_vec
                        axi_wdata <= CAN1_WRITE_CANID;                                        --BTR_VAL :  std_
                    elsif unsigned(write_index) = 2 then                             --IER_VAL :  std_     --constant TXFIFO_DW2 : std_logic_vec
                        axi_wdata <= CAN_DLC;                                  --ISR_TXOK_VAL : 
                    elsif unsigned(write_index) = 3 then                             --CAN_ID       :      --constant TXFIFO_DW2 : std_logic_vec
                        axi_wdata <= CAN1_WRITE_CANDW1;                                 --CAN_DLC      : 
                    elsif unsigned(write_index) = 4 then                             --CAN_ID       :      --constant TXFIFO_DW2 : std_logic_vec
                        axi_wdata <= CAN1_WRITE_CANDW2;                                --CAN_DATA2    :                                --CAN_DATA2    :           --   constant ADDR_TXHPB_ID  : std_lo                                                                 --   constant ADDR_TXHPB_DW1 : std_lo
                    else                                                         --CAN_DATA2    :      --   constant ADDR_TXHPB_DW2 : std_lo
                        axi_wdata <= x"10101010";                                --CAN_DATA2    :      --   constant ADDR_TXHPB_DW2 : std_lo
                    end if; 
               end if;                                                   --CAN_DATA2    : 
	    ...
	    begin                                                                              
	      if (rising_edge (M_AXI_ACLK)) then                                               
	        if (M_AXI_ARESETN = '0' or init_txn_pulse = '1' ) then                                                 
	          ...
	          if unsigned(read_index)=4 and M_AXI_RVALID = '1'  then
